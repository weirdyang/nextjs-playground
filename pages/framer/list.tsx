...
export default function () {
    ...
    function editItem(index) {
        setItems(items => {
            ...
            items[index].preview = items[index].edit
                ? items[index].text
                : '';
            return [...items]
        })
    ...
    return (
        <Layout>
            <div className="p-20 container w-4/5">
                ...
                <ul
                    className="mt-8 border rounded overflow-hidden p-8">
                            ...
                            items.map((item, index) => (
                                ...
                                >
                                    <div className='border-b flex items-center justify-between py-2'>
                                        {item.edit
                                            ? <TodoEdit item={item} updateItem={updateItem} updateText={updateText} index={index} editItem={editItem}></TodoEdit>
                                            : <Todo item={item} removeItem={removeItem} editItem={editItem} index={index}></Todo>
                                        }
                                    </div>
                                </motion.li>
                            ))
